...
module.exports = {
  scripts: {
     ...
     * yarn start "typeorm.migration.create --name migration_file_name"
     */
    typeorm: {
      migration: {
        create: {
          script: series(
            'nps config',
            'node -r ts-node/register ./node_modules/typeorm/cli.js migration:create --config ormconfig.json'
          ),
    ...
    db: {
      ...
      drop: {
        script: series(
          'nps config',
          runFast('./node_modules/typeorm/cli.js schema:drop')
        ),
...
function tslint(path) {
  return `eslint -c ./.eslintrc.js ${path} --format stylish`
}
